...
function App() {
  ...
  const getFieldList = async () => {
    ...
    const data = await response.json();
    setFields(data);
    setFieldFilters([ {id:1, disabled:false, fields:data}]);
    console.log(data);
  };
  ...
  const handleInsert = () => {
    ...
    //console.log('before fieldFilters.length:'+fieldFilters.length+' newFieldFilters.length:'+newFieldFilters.length);
    //console.log('->'+fieldFilterKey);
    //newFieldFilters.push(<FieldFilter key={fieldFilterKey} fields={fields} id={fieldFilterKey}/>);
    newFieldFilters.push({id:fieldFilterKey, disabled:true, fields:{fields}});
    //console.log('after fieldFilters.length:'+fieldFilters.length+' newFieldFilters.length:'+newFieldFilters.length);
    setFieldFilters(newFieldFilters);
    //console.log('after setting fieldFilters.length:'+fieldFilters.length+' newFieldFilters.length:'+newFieldFilters.length);
    //console.log('---->after:'+JSON.stringify(fieldFilters));
  }

  const updateFieldFilterState = (val) => {
    console.log('in updateFieldFilterState'+val);
  }
  ...
  return (
    <div className="App">
      ...
      {fields?.length > 0 ? (
        ...
        {fieldFilters.map((fieldFilter) => <FieldFilter key={fieldFilter.id} fields={fields} id={fieldFilter.id} disabled={fieldFilter.disabled} updateFieldFilterState={this.updateFieldFilterState}/>)}
        </>
